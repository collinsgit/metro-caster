...
SceneParser::parseMaterial() 
{
    ...
    filename[0] = 0;
    Vector3f diffuseColor(1,1,1), specularColor(0,0,0), transColor(0, 0, 0);
    float shininess = 0;
    float refIndex = 1;
    getToken(token); assert(!strcmp(token, "{"));
    while (true) {
    ...
    Material *answer = new Material(diffuseColor, specularColor, transColor, shininess, refIndex);
...
SceneParser::parseTorus()
{
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token); assert(!strcmp(token, "{"));
    getToken(token); assert(!strcmp(token, "R"));
    float R = readFloat();
    getToken(token); assert(!strcmp(token, "r"));
    float r = readFloat();
    getToken(token); assert(!strcmp(token, "}"));
